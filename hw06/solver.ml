...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) = struct
  let solve (g : Graph.t) : Graph.t =
    ...
    let ns = Graph.NodeS.elements nodes in
    let rec helper g ls =
      match ls with
      | [] -> g
      | l :: ls' ->
          let old_out = Graph.out g l in
          let pred = Graph.preds g l in
          let pred_facts =
            List.map (fun n -> Graph.out g n) (Graph.NodeS.elements pred)
          in
          let in_n = Fact.combine pred_facts in
          let new_fact = Graph.flow g l in_n in
          let g' = Graph.add_fact l new_fact g in
          if Fact.compare old_out new_fact != 0 then
            let succ = Graph.succs g l in
            let add_w =
              List.fold_left
                (fun acc s -> Graph.NodeS.add s acc)
                (Graph.nodes g')
                (Graph.NodeS.elements succ)
            in
            helper g' (ls' @ Graph.NodeS.elements add_w)
          else helper g' ls'
    in
    helper g ns
end
