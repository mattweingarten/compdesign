...
module SymConst = struct
  ...
  let join x y =
    match (x, y) with
    | Const a, Const b -> if a = b then Some (Const a) else Some NonConst
...
let insn_flow ((u, i) : uid * insn) (d : fact) : fact =
  let eval_op (op : operand) : SymConst.t =
    ...
    | Gid id | Id id ->
        ...
        in
        res
    | Null -> (*failwith "Cannot evaluate null in eval_op"*) SymConst.UndefConst
  in
  ...
  | Store (t, src, dst) -> UidM.add u SymConst.UndefConst d
  | Call (t, _, _) ->
      if t = Void then UidM.add u SymConst.UndefConst d
...
module Fact = struct
  ...
  let combine (ds : fact list) : fact =
    let join_const k (x : SymConst.t option) (y : SymConst.t option) :
        SymConst.t option =
      match (x, y) with
      | None, x -> x
      | x, None -> x
...
let run (cg : Graph.t) (cfg : Cfg.t) : Cfg.t =
  ...
  let cp_block (l : Ll.lbl) (cfg : Cfg.t) : Cfg.t =
    ...
    let get_sym (uid : uid) (id : uid) : SymConst.t =
      try UidM.find id (cb uid) with Not_found -> SymConst.UndefConst
    in
    let replace (uid : uid) (op : operand) : operand =
      match op with
      | Id id | Gid id -> (
          match get_sym uid id with SymConst.Const x -> Ll.Const x | _ -> op )
      | _ -> op
    in
    ...
    let replace_insn ((id, i) : uid * insn) : uid * insn =
      let new_i =
        match i with
        | Binop (bop, t, op1, op2) ->
            Binop (bop, t, replace id op1, replace id op2)
        ...
        | Call (t, op, args) ->
            Call
              ( t,
                replace id op,
                List.map (fun (id2, op) -> (id2, replace id op)) args )
    ...
    in
    let new_block =
      { insns = replace_insns b.insns; term = replace_term b.term }
    in
    let temp = LblM.remove l cfg.blocks in
    {
      blocks = LblM.add l new_block temp;
      preds = cfg.preds;
      ret_ty = cfg.ret_ty;
      args = cfg.args;
    }
